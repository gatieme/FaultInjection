	...
	for( i = 0; i < faultTable.size( ); i++ )
	{
		...
		if( faultTable[i].addr == -1 )
		{
			...
			*/
			...
			if( faultTable[i].location == text_area )       //  text segment
			{
			...
			else if( faultTable[i].location == data_area )  //  data segment
			{
			...
			else if( faultTable[i].location == stack_area ) // stack segment
			{
        ...
        printf("File %s, Line = %d, iRet =%d, fauletype = %d\n", __FILE__, __LINE__, iRet, faultTable[i].faultType);
